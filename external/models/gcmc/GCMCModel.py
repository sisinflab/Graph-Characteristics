...
class GCMCModel(torch.nn.Module, ABC):
    def __init__(self,
        ...
        self.P = torch.nn.Parameter(
            torch.nn.init.orthogonal_(
                torch.empty((self.num_basis, self.dense_layer_size[-1], self.dense_layer_size[-1]))))
        self.P.to(self.device)
        self.A = torch.nn.Parameter(torch.nn.init.uniform_(torch.empty((self.num_basis, self.num_relations, 1, 1))))
    ...
    def forward(self, inputs, **kwargs):
        ...
        for r in range(self.num_relations):
            q_r = torch.sum(torch.mul(self.A[:, r], self.P), dim=0)
