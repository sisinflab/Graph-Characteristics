...
class NameSpaceModel:
    def __init__(self, config_path, base_folder_path_elliot, base_folder_path_config, **kwargs):
        ...
        self.config['experiment']['path_output_rec_weight'] = self.config['experiment'][
            'path_output_rec_weight'].format(name_dataset_output)
        self.config['experiment']['path_output_rec_performance'] = self.config['experiment'][
            'path_output_rec_performance'].format(name_dataset_output)
        self.config['experiment']['data_config'][
            'dataset_path'] = '../data/{0}/' + f'{kwargs["sampling"]}/dataset-{kwargs["idx"]}.tsv'
            ...
            if p == _data_config:
                ...
                if side_information:
                    if isinstance(side_information, list):
                        ...
                        self.config[_experiment][p].update({k: self._safe_set_path(self._base_folder_path_config, v,
                        ...
                        side_information.update({k: self._safe_set_path(self._base_folder_path_config, v,
                                                                        self.config[_experiment][_dataset])
                ...
                if not self.config[_experiment].get(p, False):
                    setattr(self.base_namespace, p, os.path.abspath(
                        os.sep.join([self._base_folder_path_elliot, "config", "logger_config.yml"])))
                else:
                    setattr(self.base_namespace, p,
                            self._safe_set_path(self._base_folder_path_config, self.config[_experiment][p],
                                                self.config[_experiment][_dataset]))
                # setattr(self.base_namespace, p, f"{self._base_folder_path_elliot}/config/logger_config.yml")
            elif p == _log_folder:
                if not self.config[_experiment].get(p, False):
                    setattr(self.base_namespace, p,
                            os.path.abspath(os.sep.join([self._base_folder_path_elliot, "..", "log"])))
                else:
                    setattr(self.base_namespace, p,
                            self._safe_set_path(self._base_folder_path_config, self.config[_experiment][p],
                                                self.config[_experiment][_dataset]))
                ...
                self.config[_experiment][p] = self._safe_set_path(self._base_folder_path_config,
                                                                  self.config[_experiment][p], "")
                setattr(self.base_namespace, p, self.config[_experiment][p])
    ...
    def fill_model(self):
        for key in self.config[_experiment][_models]:
            ...
            if any(isinstance(value, list) for value in self.config[_experiment][_models][key].values()):
                ...
                _estimated_evals = reduce(lambda x, y: x * y, [len(param.pos_args) - 1 for _, param in _SPACE.items()],
                                          1)
                _max_evals = meta_model.get(_hyper_max_evals, _estimated_evals)
                if _max_evals <= 0:
                    raise Exception(
                        "Only pure value lists can be used without hyper_max_evals option. Please define hyper_max_evals in model/meta configuration.")
